...
if (process.argv.length < 8) {
  console.clear();
  console.log(`\n${colored(colors.COLOR_BRIGHT_RED, '                               browser of gay man remake for compatible with flood ....    ')}`);
  console.log('');
  console.log(colored(colors.COLOR_CYAN, "       gay man browser "));
  console.log('');
  ...
  console.log(`
    ...
    ${colored(colors.COLOR_BRIGHT_BLUE, 'Usage:')}
        ...
        node browser.js https://example.com 8 proxy.txt 64 200  --fin true --load true --headers true --blocked Indonesia --reconnect true --ipv6 true
...
await sleep(10);
    try {
        ...
        if (isSolved) {
            console.log('Geetest slide dã du?c x? lý thành công.');
        } else {
            console.log('X? lý Geetest slide không thành công.');
        }
    } catch (error) {
        console.error('L?i khi x? lý Geetest slide:', error);
    }
